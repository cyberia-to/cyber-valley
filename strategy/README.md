...
The master developer thesis is simple: **land is the capital, infrastructure is the bottleneck, sequencing is the strategy.** Early transactions must finance infrastructure without giving away the upside; infrastructure then multiplies the value of everything sold later. The reference playbook, oldest lesson first: [Grosvenor Estate](https://en.wikipedia.org/wiki/Grosvenor_Group) — London ground leases since 1677, never selling the freehold outlives every market cycle; [Letchworth](https://en.wikipedia.org/wiki/Letchworth) — the first garden city, whose land trust still funds the town from ground rents 120 years on, the community-land-trust endgame of §7; [Irvine Ranch](https://en.wikipedia.org/wiki/Irvine_Ranch) — 93,000 acres under one private owner since 1960, income land never sold, more than half the ranch preserved as the amenity engine that prices the rest; [MTR rail-plus-property](https://en.wikipedia.org/wiki/MTR) — infrastructure ownership as the value-capture machine. Two cautionary tales bound the playbook: Irvine's residential ground-rent resets sparked a homeowner revolt in the 1970s — which is why the [[annual leasehold]] schedule is fixed at signing, never reset by surprise; and [Celebration](https://en.wikipedia.org/wiki/Celebration,_Florida) proved engineered tone sells but decays the moment the developer exits stewardship — stewardship here never exits.
